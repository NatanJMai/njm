...
void vTaskFunction( void *pvParameters )
{
    ...
    if (xQueue1 != NULL){
        pxMessage = &xMessage;
        my_xQueueSend(xQueue1, (void *) &pxMessage, (TickType_t) 0);
        vPrintMsg(xMessage.ucData);
        check = simpleCheckSum(xMessage.ucData, myStrlen(xMessage.ucData));
        vPrintMsg(myAtoi(check));
    ...
    for( ; ; )
    {
        ...
        if (xQueue1 != NULL){
            if(xQueueReceive(xQueue1, &(pxRxedMessage),(TickType_t) 10)){
                vPrintMsg("Mensagem recebida -> "), vPrintMsg(pxRxedMessage->ucData), check = simpleCheckSum(pxRxedMessage->ucData, myStrlen(pxRxedMessage->ucData));
                vPrintMsg(myAtoi(check));
                vPrintMsg("\n");
            }
        }
...
static const paramStruct tParam[2] =
{
    ...
    (paramStruct) { .text="Task2 - Periodic task\r\n", .delay=3000 }
};
...
void main(void)
{
     ...
     */
    vDirectPrintMsg("= = = T E S T   S T A R T E De = = =\r\n\r\n");
